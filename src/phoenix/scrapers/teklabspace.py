"""TekLabSpace scraper — Tier 3 (Playwright, SPA).

Leaderboard: https://app.teklabspace.com/leaders-board/
Profile: https://app.teklabspace.com/profile/{username}
"""
...
LEADERBOARD_URL = "https://app.teklabspace.com/leaders-board/"
PROFILE_BASE = "https://app.teklabspace.com/profile"


@register_scraper("teklabspace")
class TeklabspaceScraper(PlaywrightScraper):
    platform_name = "teklabspace"

    async def scrape_leaderboard(self, max_entries: int = 100) -> list[LeaderboardEntry]:
        log.warning(
            "teklabspace_dns_dead",
            msg="TekLabSpace app.teklabspace.com domain no longer resolves (DNS failure). "
                "Site appears to be decommissioned.",
        )
        return []
    ...
    async def scrape_profile(self, username: str) -> tuple[PlatformProfile, ProfileSnapshot]:
        ...
        try:
            ...
            external_links = await self._get_all_links(page, exclude_domain="teklabspace.com")
