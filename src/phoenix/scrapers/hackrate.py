"""HackRate scraper — Tier 3 (Playwright, SPA).

Leaderboard: https://hckrt.com/Profiles/Leaderboard
Profile: https://hckrt.com/Profiles/{username}

The leaderboard requires authentication. Scraping is not possible
without logged-in session credentials.
"""
...
LEADERBOARD_URL = "https://hckrt.com/Profiles/Leaderboard"
PROFILE_BASE = "https://hckrt.com/Profiles"


@register_scraper("hackrate")
class HackrateScraper(PlaywrightScraper):
    ...
    async def scrape_profile(self, username: str) -> tuple[PlatformProfile, ProfileSnapshot]:
        ...
        try:
            ...
            score_match = re.search(r"(?:Points|Score|Reputation|Rating)\s*[:#]?\s*([\d,]+(?:\.\d+)?)", body, re.I)
            if score_match:
            ...
            programs_match = re.search(r"(?:Programs?|Projects?)\s*[:#]?\s*(\d+)", body, re.I)
            if programs_match:
            ...
            external_links = await self._get_all_links(page, exclude_domain="hckrt.com")
